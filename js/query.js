...
fetch('https://wax.api.atomicassets.io/atomicmarket/v1/stats/graph?symbol=WAX&collection_whitelist=samuraiworld')
   ...
   .then(response => {
      ...
      document.querySelector('#dayVol').innerHTML = `<h5 style="color: white"><i class="fas fa-chart-line" style="color: lightgreen"></i> Yesterday Total Volume : <span style="color: lightgreen">${dayVolume} WAX</span></h5>`;
   });
fetch('https://wax.api.atomicassets.io/atomicmarket/v1/stats/sales?symbol=WAX&collection_whitelist=samuraiworld')
   ...
   .then(response => {
      ...
      document.querySelector('#totalVol').innerHTML = `<h5 style="color: white"><i class="fas fa-chart-line" style="color: lightgreen"></i> Current Total Volume : <span style="color: lightgreen">${totalVolume} WAX</span></h5>`;
   });